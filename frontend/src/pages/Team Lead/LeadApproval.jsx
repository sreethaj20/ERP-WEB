...
export default function LeaveRequests() {
  ...
  const getDirectReports = (allUsers = []) => {
    ...
    const matchesMe = (raw) => {
      ...
      return (meEmail && vLower === meEmail) || (meName && vLower === meName);
    };

    const onlyEmployees = (allUsers || []).filter(u => String(u?.role || '').toLowerCase() === 'employee');
    return onlyEmployees.filter((u) => {
  ...
  const mapRows = (rows = []) =>
    rows.map(r => ({
      ...
      startDate: r.from,
      endDate: r.to,
      days: (r.duration != null ? Number(r.duration) : Math.max(1, Math.ceil((new Date(r.to) - new Date(r.from)) / (1000*60*60*24)) + 1)),
  ...
  // Load team members for the current team lead
  const loadTeamMembers = async () => {
    try {
      if (isHRBPLeadUser) {
        const members = getDirectReports(Array.isArray(users) ? users : []);
        setTeamMembers(members || []);
        return members || [];
      }
      const members = await getMyTeamUsers();
      setTeamMembers(members || []);
      return members || [];
    } catch (error) {
  ...
  // Load all leave requests and filter by team members
        ...
        <table className="leave-requests-table">
          <thead>
            <tr>
              ...
              <th style={{ width: '100px' }}>Team Lead</th>
...
//                     No pending team leave requests found.
//                   </td>
//                 </tr>
//               ) : (
//                 filtered.map((request) => (
...
//                         <button 
//                           className="btn-primary"
//                           onClick={() => updateStatus(request.id, 'approved', 'Team Lead')}
//                           style={{ fontSize: '10px', padding: '3px 6px', minHeight: '24px' }}
//                         >
//                           Approve
//                         </button>
//                         <button 
//                           className="btn-outline"
//                           onClick={() => updateStatus(request.id, 'rejected', 'Team Lead')}
//                           style={{ fontSize: '10px', padding: '3px 6px', minHeight: '24px' }}
//                         >
//                           Reject
//                         </button>
